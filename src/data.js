...
function buildProducts() {
  ...
  return catalog.map((item, i) => {
    ...
    return {
      ...
      // Витринный товар "для вида" — не привязан ни к одному продавцу, так
      // что заказать его по-настоящему нельзя (некому подтвердить наличие
      // и отдать). См. isOrderable() в store.jsx.
      isDemo: true,
    };
  });
}
